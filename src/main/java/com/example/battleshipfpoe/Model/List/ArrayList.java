...
public class ArrayList<T extends Serializable> implements IList<T>, Serializable {
    private T[] array;
    private int size;
    ...
    public ArrayList() {
        array = (T[]) new Serializable[INITIAL_CAPACITY]; // Use Serializable[] here
        size = 0;
    }
    ...
    public ArrayList<T> clone() {
        ...
        // Perform deep copy of all elements in the array
        for (int i = 0; i < size; i++) {
            clonedList.addLast(array[i]);
        }

        return clonedList;
    }

    // Existing deepCopy() method (reusing it for clone)
    @Override
    ...
    private void ensureCapacity(int minCapacity) {
        if (minCapacity > array.length) {
            ...
            @SuppressWarnings("unchecked")
            T[] newArray = (T[]) new Serializable[newCapacity]; // Use Serializable[] here
